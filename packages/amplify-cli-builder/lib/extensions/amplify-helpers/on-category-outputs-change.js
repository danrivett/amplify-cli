...
async function onCategoryOutputsChange(context, cloudAmplifyMeta, localMeta) {
  ...
  if (projectConfig.frontend) {
    ...
    await frontendHandlerModule.createFrontendConfigs(
      context,
      (0, get_resource_outputs_1.getResourceOutputs)(localMeta),
      (0, get_resource_outputs_1.getResourceOutputs)(cloudAmplifyMeta),
    );
  }
...
function ensureAmplifyMetaFrontendConfig(amplifyMeta) {
  var _a;
  var _b;
  if (!amplifyMeta) {
  ...
  const authResourceName = Object.keys(amplifyMeta.auth).find(key => {
