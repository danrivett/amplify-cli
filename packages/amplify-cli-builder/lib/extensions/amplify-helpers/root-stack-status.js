...
async function isRootStackModifiedSinceLastPush(hashFunction) {
  try {
    ...
    if (
      fs.existsSync(localBackendDir) &&
      fs.existsSync(path.join(localBackendDir, amplify_provider_awscloudformation_1.rootStackFileName))
    ) {
      const localCfnBuffer = fs.readFileSync(path.join(localBackendDir, amplify_provider_awscloudformation_1.rootStackFileName));
      if (
        fs.existsSync(cloudBackendDir) &&
        fs.existsSync(path.join(cloudBackendDir, amplify_provider_awscloudformation_1.rootStackFileName))
      ) {
