...
exports.updateamplifyMetaAfterResourceDelete =
  exports.updateAmplifyMetaAfterPackage =
  exports.updateamplifyMetaAfterBuild =
  exports.updateamplifyMetaAfterPush =
  exports.updateamplifyMetaAfterResourceUpdate =
  exports.updateProvideramplifyMeta =
  exports.updateamplifyMetaAfterResourceAdd =
  exports.updateAwsMetaFile =
    void 0;
...
function moveBackendResourcesToCurrentCloudBackend(resources) {
  ...
  const overridePackageJsonCurrentCloudBackendFilePath = path.join(
    amplify_cli_core_1.pathManager.getCurrentCloudBackendDirPath(),
    'package.json',
  );
  const overrideTsConfigJsonCurrentCloudBackendFilePath = path.join(
    amplify_cli_core_1.pathManager.getCurrentCloudBackendDirPath(),
    'tsconfig.json',
  );
  for (const resource of resources) {
    ...
    if (fs.pathExistsSync(sourceDir)) {
      ...
      if (
        (resource === null || resource === void 0 ? void 0 : resource.service) === 'Lambda' ||
        ((resource === null || resource === void 0 ? void 0 : resource.service) &&
          (resource === null || resource === void 0 ? void 0 : resource.service.includes('custom')))
      ) {
      ...
      if (fs.pathExistsSync(sourceDir)) {
        ...
        if (resource.category === 'hosting' && resource.service === 'ElasticContainer') {
          ...
          if (projectRootPath) {
            ...
            hashDir = await (0, resource_status_1.getHashForResourceDir)(sourceAbsolutePath, [
              'Dockerfile',
              'docker-compose.yaml',
              'docker-compose.yml',
            ]);
...
function updateamplifyMetaAfterBuild({ category, resourceName }, buildType = amplify_function_plugin_interface_1.BuildType.PROD) {
  ...
  lodash_1.default.set(amplifyMeta, [category, resourceName, 'lastBuildType'], buildType);
  amplify_cli_core_1.stateManager.setMeta(undefined, amplifyMeta);
}
exports.updateamplifyMetaAfterBuild = updateamplifyMetaAfterBuild;
function updateAmplifyMetaAfterPackage({ category, resourceName }, zipFilename, hash) {
  const amplifyMeta = amplify_cli_core_1.stateManager.getMeta();
  ...
  if (hash) {
    lodash_1.default.set(amplifyMeta, [category, resourceName, hash.resourceKey], hash.hashValue);
  }
  amplify_cli_core_1.stateManager.setMeta(undefined, amplifyMeta);
}
