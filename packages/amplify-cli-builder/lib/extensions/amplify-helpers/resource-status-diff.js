...
function globCFNFilePath(fileFolder) {
...
class ResourceDiff {
  constructor(category, resourceName, provider, mutationInfo) {
    this.printResourceDetailStatus = async mutationInfo => {
      ...
      print_1.print.info(
        `${resourceDetailSectionStyle(`[\u27A5] Resource Stack: ${capitalize(this.category)}/${this.resourceName}`)} : ${header}`,
      );
    ...
    this.calculateCfnDiff = async () => {
      ...
      this.localTemplate = (0, amplify_cli_core_1.generateCustomPoliciesInTemplate)(
        this.localTemplate,
        this.resourceName,
        this.service,
        this.category,
  ...
  normalizeProviderForFileNames(provider) {
  ...
  safeGlobCFNFilePath(fileFolder) {
    try {
  ...
  isResourceTypeCDKMetada(resourceType) {
...
async function CollateResourceDiffs(resources, mutationInfo) {
