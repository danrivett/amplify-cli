...
const getConfiguredProviders = context => {
  ...
  return lodash_1.default.pick(getProviderPlugins(context), configuredProviders);
};
exports.getConfiguredProviders = getConfiguredProviders;
const executeProviderCommand = async (context, command, args = []) => {
  const providers = await Promise.all(
    Object.values((0, exports.getConfiguredProviders)(context)).map(providerPath =>
      Promise.resolve().then(() => __importStar(require(providerPath))),
    ),
  );
  await Promise.all(
    providers
      .filter(provider => typeof (provider === null || provider === void 0 ? void 0 : provider[command]) === 'function')
      .map(provider => provider[command](context, ...args)),
  );
};
