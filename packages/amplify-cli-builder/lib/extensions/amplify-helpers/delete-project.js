...
async function deleteProject(context) {
  ...
  if (confirmation.proceed) {
    ...
    const spinner = (0, ora_1.default)('Deleting resources from the cloud. This may take a few minutes...');
    try {
      spinner.start();
      await Promise.all(
        Object.keys(allEnvs).map(env => (0, remove_env_from_cloud_1.removeEnvFromCloud)(context, env, confirmation.deleteS3)),
      );
