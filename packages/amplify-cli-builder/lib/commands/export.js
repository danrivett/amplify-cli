...
const run = async context => {
  ...
  if (showPullHelp) {
    ...
    amplify_prompts_1.printer.blankLine();
    amplify_prompts_1.printer.info("'amplify export pull', Allows you to genreate frontend config files at a desired location");
    amplify_prompts_1.printer.blankLine();
...
async function createFrontEndConfigFile(context, exportPath) {
  ...
  try {
    ...
    const validatedExportPath = (0, amplify_cli_core_1.validateExportDirectoryPath)(
      exportPath,
      amplify_cli_core_1.PathConstants.DefaultFrontEndExportFolder,
    );
    await frontendHandlerModule.createFrontendConfigsAtPath(
      context,
      (0, get_resource_outputs_1.getResourceOutputs)(meta),
      (0, get_resource_outputs_1.getResourceOutputs)(cloudMeta),
      validatedExportPath,
    );
